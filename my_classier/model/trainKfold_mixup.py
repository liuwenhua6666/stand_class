...
def train_model(model, criterion, optimizer, lr_scheduler, num_epochs=NUM_EPOCHS,model_name=None,train_dir=None,val_dir=None):
    ...
    for epoch in range(1,NUM_EPOCHS+1):
        ...
        for i, data in enumerate(data_loaders):
            ...
            inputs_org, labels = inputs.cuda(), labels.cuda()
            pro = np.random.randint(0, 100)
            if pro < 20:
                inputs_mixup, targets_a, targets_b, lam = mixup_data(inputs_org, labels,
                                                           0.1, True)
...
if __name__ == "__main__":
    ...
    # model_list = ['resnet18']
    model_list = ['resnet18','resnet34','se_resnext50_32x4d']
